...
/* 조직도 개선: CON-COST 우측 보조조직 배치 + 기술본부 중심 배치 + 중분류 구조 */
.org-concost-fit{
  display:grid;
  gap:14px;
  min-height:560px;
...
.org-concost-body{
  display:grid;
  grid-template-columns:290px minmax(760px,1fr) 290px;
  gap:12px;
  align-items:stretch;
  min-height:430px;
}
...
.org-subgroup-members{
  display:grid;
  grid-template-columns:repeat(2,minmax(118px,1fr));
  gap:6px;
}

.org-subgroup-members > .org-subgroup-card{
  grid-column:1/-1;
}

.org-overview-fit-nested .nested-grid{
  grid-template-columns:1fr 1fr;
}

.org-overview-fit-nested .org-overview-card.nested:nth-child(1),
.org-overview-fit-nested .org-overview-card.nested:nth-child(2){
  min-height:220px;
}

.org-overview-fit-nested .org-overview-card.nested:nth-child(3),
.org-overview-fit-nested .org-overview-card.nested:nth-child(4){
  min-height:360px;
}

.org-mini-person.dotted{
  background:#f1f5f9;
  border-color:#cbd5e1;
}
...
@media (max-width:1500px){
  .org-concost-body{
    grid-template-columns:260px minmax(680px,1fr) 260px;
  }
