...
(function installQcReviewProjectAccessPatch(){
  ...
  window.openQcProjectStageMenuWindow = function openQcProjectStageMenuWindow(){
    ...
    const popupWidth = Math.max(1680, Math.min(screen.availWidth || 1920, 1880));
    const popupHeight = Math.max(900, Math.min(screen.availHeight || 1080, 980));
    const win = window.open("", `qc_stage_inline_${projectKey}`.replace(/[^a-zA-Z0-9_]/g, "_"), `width=${popupWidth},height=${popupHeight},left=20,top=20,scrollbars=yes,resizable=yes`);
    ...
    doc.write(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>${qcEscape(projectLabel)} · 구분 별 리스트</title>
      <style>
        ...
        .stage-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:8px}.stage-btn{border:1px solid #dbe3ef;background:#fff;border-radius:12px;padding:8px 8px;min-height:54px;cursor:pointer;font-weight:900;text-align:center;line-height:1.25}.stage-btn:hover,.stage-btn.active{border-color:#2563eb;background:#eff6ff;box-shadow:0 0 0 3px rgba(37,99,235,.10)}.stage-btn span{display:block;margin-top:4px;color:#2563eb;font-size:12px}
