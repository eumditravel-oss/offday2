...
/* 2026-06-15 QC review category filter redesign */
(function installChecklistCategoryFilterRedesign(){
  function qcFilterText(value) {
    return String(value ?? "").replace(/\s+/g, " ").trim();
  }

  function qcFilterDefaultText(value, fallback = "전체") {
    const text = qcFilterText(value);
    return text || fallback;
  }

  function qcFilterCategoryCount(category) {
    return category === "전체" ? checklistRows.length : getChecklistCategoryCount(category);
  }

  function qcFilterCurrentProjectLabel() {
    return qcFilterDefaultText(document.getElementById("checklistProject")?.value, "프로젝트 미선택");
  }

  function qcFilterMiddleLabel() {
    if (!Array.isArray(checklistMiddleFilterOptions) || !checklistMiddleFilterOptions.length) return "중분류 전체";
    const active = checklistMiddleFilterOptions.filter(option => selectedChecklistMiddleFilters.has(option));
    if (!active.length) return "중분류 없음";
    if (active.length === checklistMiddleFilterOptions.length) return "중분류 전체";
    return active.join(", ");
  ...
  function qcFilterStatusChips(activeLabel, activeCount) {
    const owner = qcFilterDefaultText(document.getElementById("checklistOwnerFilter")?.value, "전체");
    const done = qcFilterDefaultText(document.getElementById("checklistDoneFilter")?.value, "전체");
    const search = qcFilterText(document.getElementById("checklistSearch")?.value);
    const chips = [
      `<span class="qc-filter-status-chip strong"><b>구분</b>${escapeHtml(activeLabel)} · ${activeCount}</span>`,
      `<span class="qc-filter-status-chip"><b>중분류</b>${escapeHtml(qcFilterMiddleLabel())}</span>`
    ];
    if (owner !== "전체") chips.push(`<span class="qc-filter-status-chip"><b>대상</b>${escapeHtml(owner)}</span>`);
    if (done !== "전체") chips.push(`<span class="qc-filter-status-chip"><b>체크</b>${escapeHtml(done)}</span>`);
    if (search) chips.push(`<span class="qc-filter-status-chip"><b>검색</b>${escapeHtml(search)}</span>`);
  ...
  function qcFilterCategoryGroup(category) {
    const label = getChecklistCategoryLabel(category);
    if (category === "전체") return "all";
    if (/견적조건|최종/.test(label) && /^Z?7|\(최종\)/.test(label)) return "final";
    if (/질의사항|\bZ[1-6]\./.test(label)) return "question";
    if (/검토/.test(label)) return "review";
    return "flow";
  }

  function qcFilterCategoryButton(category, mode = "") {
    const active = selectedChecklistCategoryFilter === category ? "active" : "";
    const count = qcFilterCategoryCount(category);
    return `<button type="button" class="qc-category-pill ${mode} ${active}" onclick="setChecklistCategoryFilter('${escapeJs(category)}')">
      <span>${escapeHtml(getChecklistCategoryLabel(category))}</span>
      <em>${count}</em>
    </button>`;
  ...
  function qcFilterLane(title, categories, tone = "") {
    if (!categories.length) return "";
    return `<section class="qc-category-lane ${tone}">
      <h3>${escapeHtml(title)}</h3>
      <div class="qc-category-lane-items">${categories.map(category => qcFilterCategoryButton(category)).join("")}</div>
    </section>`;
  }

  function qcFilterLanes(categories) {
    const lanes = {
      flow: [],
      review: [],
      question: [],
    ...
    categories.forEach(category => {
      const group = qcFilterCategoryGroup(category);
      if (group !== "all" && lanes[group]) lanes[group].push(category);
    });
    return [
      qcFilterLane("진행", lanes.flow, "flow"),
      qcFilterLane("검토", lanes.review, "review"),
      qcFilterLane("질의", lanes.question, "question"),
      qcFilterLane("최종", lanes.final, "final")
    ].filter(Boolean).join("");
  }

  renderChecklistCategoryButtons = function renderChecklistCategoryButtonsRedesigned() {
    const wrap = document.getElementById("checklistCategoryFilter");
    if (!wrap) return;
    ...
    const activeLabel = getChecklistCategoryLabel(selectedChecklistCategoryFilter || "전체");
    const activeCount = qcFilterCategoryCount(selectedChecklistCategoryFilter || "전체");
    const allVisibleCollapsed = areAllVisibleChecklistGroupsCollapsed();
    const detailButtonLabel = allVisibleCollapsed ? "목록 펼치기" : "목록 접기";
    const lanes = qcFilterLanes(visibleCategories);
    ...
    wrap.innerHTML = `
      <div class="checklist-filter-shell qc-filter-redesign ${checklistCategoryPanelOpen ? "open" : ""}">
        <div class="qc-filter-topbar">
          <div class="qc-filter-project">
            <span>프로젝트</span>
            <strong>${escapeHtml(qcFilterCurrentProjectLabel())}</strong>
          </div>
          <div class="qc-filter-actions">
            <button type="button" class="qc-filter-quiet-btn" onclick="resetChecklistReadabilityFilters()">초기화</button>
            <button type="button" class="qc-filter-quiet-btn" onclick="event.stopPropagation(); toggleChecklistDetailVisibility()">${detailButtonLabel}</button>
          </div>
        </div>
        <div class="qc-filter-overview">
          ${qcFilterCategoryButton("전체", "all")}
          <div class="qc-filter-current">
            <span>현재 필터</span>
            <div class="qc-filter-status-row">${qcFilterStatusChips(activeLabel, activeCount)}</div>
          </div>
        </div>
        <div class="qc-filter-middle-wrap">
          ${renderChecklistMiddleFilterBar()}
        </div>
        <div class="qc-category-lanes ${checklistCategoryPanelOpen ? "open" : ""}">
          ${lanes}
        </div>
      </div>`;
  };
