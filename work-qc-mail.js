...
/* =========================================================
   QC 프로젝트 접속 경로 + 기존 체크리스트 새 창 실행 보강
   기준: data1037 구조 유지, 기존 renderChecklistGrid 계열 함수 재사용
========================================================= */
(function(){
  ...
  function qcProjectKey(data){ return qcText(data?.projectNo || data?.pjNo || data?.dbPjNo || data?.receivedId || data?.id || ''); }
  function qcProjectName(data){ return qcText(data?.projectName || data?.name || data?.project || 'ㅇㅇ시설 신축공사'); }
  function qcProjectClient(data){ return 'ㅇㅇ건설'; }
  function qcProjectLabel(data){
    const no = qcProjectKey(data);
    const name = qcProjectName(data);
    const client = qcProjectClient(data);
    return [no, name, client].filter(Boolean).join(' · ');
  }
  function qcCollectProjectItems(){
    const map = new Map();
    const add = (raw) => {
      const data = raw?.data || raw || {};
      const key = qcProjectKey(data) || qcProjectName(data);
      if (!key || map.has(key)) return;
      map.set(key, {
        projectNo: qcProjectKey(data) || key,
        projectName: qcProjectName(data),
        client: qcProjectClient(data),
        status: qcText(data.status || data.progress || '진행중') || '진행중'
      });
    };
    try { (window.projectReceiveCompletedProjects || []).forEach(add); } catch(_) {}
    try { (window.projectReceiveList || []).forEach(add); } catch(_) {}
    try { (window.pmScheduleProjects || []).forEach(item => add(item?.project || item)); } catch(_) {}
    ...
    return Array.from(map.values()).map((item, idx) => ({
      ...item,
      projectNo: qcProjectKey(item) || String(2026001 + idx)
    }));
  }
  function qcFindProjectByText(value){
    const text = qcText(value).toLowerCase();
    if (!text) return null;
    return qcCollectProjectItems().find(item => {
      const hay = `${item.projectNo} ${item.projectName} ${item.client}`.toLowerCase();
      return hay.includes(text) || text.includes(qcText(item.projectNo).toLowerCase()) || text.includes(qcText(item.projectName).toLowerCase());
    }) || null;
  }
  function qcApplySelectedProject(data, openWindow){
    const project = data || qcFindProjectByText(document.getElementById('checklistProject')?.value || '') || qcCollectProjectItems()[0];
    const input = document.getElementById('checklistProject');
    ...
    refreshChecklistProjectRoutes();
    if (openWindow) openChecklistProjectReviewWindow(project);
  }
  ...
  window.refreshChecklistProjectRoutes = function refreshChecklistProjectRoutes(){
    ...
    const rows = projects.slice(0, 12).map((item, idx) => {
      const active = qcProjectKey(item) === qcProjectKey(selected) ? 'active' : '';
    ...
    wrap.innerHTML = `
      <div class="qc-project-route-card">
        <div class="qc-route-head">
          <div><strong>프로젝트 질의응답 접속 경로</strong><span>기존 프로젝트 검색 또는 현재 진행중인 프로젝트 리스트에서 선택 후 새 창으로 엽니다.</span></div>
          <button type="button" class="btn btn-primary" onclick="openChecklistSelectedProjectWindow()">선택 프로젝트 새 창 열기</button>
        </div>
        <div class="qc-route-grid">
          <section class="qc-route-section">
            <h3>1. 프로젝트 검색</h3>
            <p>PJ NO, received, 프로젝트명, 업체명으로 검색 후 상단의 프로젝트 선택 버튼을 누릅니다.</p>
            <div class="qc-route-selected">현재 선택: <b>${escapeHtml(qcProjectLabel(selected || {}))}</b></div>
          </section>
          <section class="qc-route-section">
            <h3>2. 현재 진행중인 프로젝트</h3>
            <p>리스트 클릭 후 프로젝트 선택 버튼을 누르면 동일한 새 창이 열립니다.</p>
            <div class="qc-current-project-list">${rows}</div>
          </section>
        </div>
      </div>`;
  };
  ...
  window.selectChecklistActiveProject = function selectChecklistActiveProject(index){
    ...
    qcApplySelectedProject(project, false);
  };

  window.openChecklistSelectedProjectWindow = function openChecklistSelectedProjectWindow(){
    const project = qcFindProjectByText(document.getElementById('checklistProject')?.value || '') || qcCollectProjectItems()[0];
    qcApplySelectedProject(project, true);
  };

  function withPopupProjectFilter(project, category, callback){
    const input = document.getElementById('checklistProject');
    const oldInput = input ? input.value : '';
    let oldCategory = null;
    try { oldCategory = selectedChecklistCategoryFilter; } catch(_) {}
    if (input) input.value = qcProjectLabel(project);
    try { selectedChecklistCategoryFilter = category || '전체'; } catch(_) {}
    try { return callback(); }
    finally {
      if (input) input.value = oldInput;
      if (oldCategory !== null) { try { selectedChecklistCategoryFilter = oldCategory; } catch(_) {} }
    }
  }
  ...
  function buildPopupRowsHtml(project, category){
    return withPopupProjectFilter(project, category || '전체', () => {
      const rows = getChecklistFilteredRows().sort((a, b) => {
        const ai = checklistCategoryOptions.indexOf(normalizeChecklistGroupName(a.row.group));
        const bi = checklistCategoryOptions.indexOf(normalizeChecklistGroupName(b.row.group));
        const ag = ai < 0 ? 999 : ai;
      ...
      rows.forEach(({ row, realIndex }) => {
        ...
        const middleKey = `${normalizedGroup}::${middle}`;
        const subKey = `${middleKey}::${sub}`;
        if (normalizedGroup !== lastGroup) { html.push(renderChecklistGroupBand(normalizedGroup)); lastGroup = normalizedGroup; lastMiddleKey = ''; lastSubKey = ''; }
        if (collapsedChecklistGroups.has(normalizedGroup)) return;
        if (middleKey !== lastMiddleKey) { html.push(renderChecklistMiddleBand(normalizedGroup, middle)); lastMiddleKey = middleKey; lastSubKey = ''; }
        if (collapsedChecklistMiddles.has(getChecklistMiddleCollapseKey(normalizedGroup, middle))) return;
        if (sub && subKey !== lastSubKey) { html.push(renderChecklistSubBand(normalizedGroup, middle, sub)); lastSubKey = subKey; }
        if (sub && collapsedChecklistSubs.has(getChecklistSubCollapseKey(normalizedGroup, middle, sub))) return;
        html.push(`
          <tr class="checklist-detail-row ${row.done ? 'row-done' : ''} ${locked ? 'locked-row' : ''} ${row.eliminated ? 'eliminated-row' : ''}" data-row-index="${realIndex}">
            <td><input type="checkbox" ${row.checked ? 'checked' : ''} ${locked ? 'disabled' : ''} onchange="updateChecklistCheck(${realIndex}, this.checked)" title="행 선택"></td>
      ...
      if (!html.length) html.push(`<tr><td colspan="11"><div class="empty-mail-box">선택 프로젝트에 표시할 항목이 없습니다.</div></td></tr>`);
      return html.join('');
    });
  }
  ...
  function installPopupWrappers(popup, project){
    const rerender = () => renderPopupWindow(popup, project, popup.__checklistPopupCategory || firstCategoryName);
    const call = (name, rerenderAfter = true) => (...args) => {
      const result = typeof window[name] === 'function' ? window[name](...args) : undefined;
      if (rerenderAfter) setTimeout(rerender, 0);
      return result;
    };
    popup.setChecklistPopupCategory = (category) => { popup.__checklistPopupCategory = category; rerender(); };
    popup.updateChecklistCell = (idx, key, value) => { updateChecklistCell(idx, key, value); };
    popup.updateChecklistCheck = (idx, checked) => { updateChecklistCheck(idx, checked); };
    popup.setChecklistCellFocus = (el) => { popup.__currentChecklistFocus = { row:Number(el.dataset.row), field:el.dataset.field }; };
    popup.moveChecklistCell = (event, el) => {
      const keys = ['Enter','ArrowDown','ArrowUp','ArrowRight','ArrowLeft','Tab'];
      if (!keys.includes(event.key) || event.shiftKey || event.ctrlKey || event.metaKey || event.altKey) return;
      const currentRow = Number(el.dataset.row); const currentField = el.dataset.field; const fields = ['trade','no','item','method','comment']; const fi = fields.indexOf(currentField);
      if (!Number.isInteger(currentRow) || fi < 0) return;
      event.preventDefault(); updateChecklistCell(currentRow,currentField,el.innerText);
      const visibleRows = Array.from(popup.document.querySelectorAll('.excel-editable-cell[data-row][data-field="trade"]')).map(node => Number(node.dataset.row));
      const pos = visibleRows.indexOf(currentRow); let targetRow=currentRow, targetField=currentField;
      if (event.key === 'Enter' || event.key === 'ArrowDown') targetRow = visibleRows[pos+1] ?? currentRow;
      else if (event.key === 'ArrowUp') targetRow = visibleRows[pos-1] ?? currentRow;
      else if (event.key === 'ArrowRight' || event.key === 'Tab') { if (fi < fields.length-1) targetField = fields[fi+1]; else { targetRow = visibleRows[pos+1] ?? currentRow; targetField = fields[0]; } }
      else if (event.key === 'ArrowLeft') { if (fi > 0) targetField = fields[fi-1]; else { targetRow = visibleRows[pos-1] ?? currentRow; targetField = fields[fields.length-1]; } }
      const target = popup.document.querySelector(`.excel-editable-cell[data-row="${targetRow}"][data-field="${targetField}"]`);
      if (target) { target.focus(); const range = popup.document.createRange(); range.selectNodeContents(target); range.collapse(false); const sel = popup.getSelection(); sel.removeAllRanges(); sel.addRange(range); }
    };
    ['toggleChecklistGroupCollapse','toggleChecklistMiddleCollapse','toggleChecklistSubCollapse','openChecklistModal','deleteChecklistRow','openChecklistClassifyModal','openChecklistTargetModal','openChecklistCheckWindow','openChecklistHistoryWindow','openChecklistAttachmentModal','toggleChecklistDone','toggleChecklistNotApplicable','runChecklistTranslation','duplicateCheckedRows','deleteCheckedRows','insertChecklistRowInGroup'].forEach(name => popup[name] = call(name, true));
    popup.renderChecklistGrid = rerender;
    popup.openChecklistModal = call('openChecklistModal', false);
    popup.openChecklistTargetModal = call('openChecklistTargetModal', false);
    popup.openChecklistClassifyModal = call('openChecklistClassifyModal', false);
    popup.openChecklistCheckWindow = call('openChecklistCheckWindow', false);
    popup.openChecklistHistoryWindow = call('openChecklistHistoryWindow', false);
    popup.openChecklistAttachmentModal = call('openChecklistAttachmentModal', false);
  }

  function renderPopupWindow(popup, project, category){
    ...
    if (title) {
      ...
      title.innerHTML = `${escapeHtml(getChecklistCategoryLabel(active))} <span>현재 ${count}건</span>`;
    }
  }

  window.openChecklistProjectReviewWindow = function openChecklistProjectReviewWindow(projectRaw){
    const project = projectRaw || qcFindProjectByText(document.getElementById('checklistProject')?.value || '') || qcCollectProjectItems()[0];
    const width = Math.min(Math.max(screen.availWidth - 80, 1500), 2100);
    const height = Math.min(Math.max(screen.availHeight - 80, 850), 1200);
  ...
  if (!window.__qcProjectRouteRenderHooked) {
    ...
    window.renderChecklistGrid = function renderChecklistGridWithProjectRoute(){
      ...
      try { refreshChecklistProjectRoutes(); } catch(_) {}
      return result;
    };
